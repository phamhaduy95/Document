...
- [x] Increase the number or read capacity units (RCUs) that are provisioned for the DynamoDB table.
...
- [x] Publish a custom metric on Amazon CloudWatch and use Amazon Simple Email Service (SES) for notification.
...
- [x] Disable the stage transition to allow manual approval.
...
- [x] An Amazon Cloudwatch NetworkIn metric.
...
- [x] Amazon SNS with fanout to an SQS queue for each application.
...
- [ ] Rolling.
- [ ] All at once.
- [x] Rolling with an additional batch.
- [ ] Immutable.
...
- [x] Request a token from AWS STS, and perform a describe action.
...
- [x] Use `InputPath` in a `Catch` statement and set the value to `null`.
...
- [x] Set `ECS_ENABLE_TASK_IAM_ROLE` to `false` on EC2 instance boot in ECS agent configuration file. Run the first microservice with an IAM role for ECS tasks with read-only access for the Aurora database. Run the second microservice with an IAM role for ECS tasks with read-only access to DynamoDB.
...
- [x] Enable Amazon CloudWatch Logs for the Lambda function. Enable execution logs for API Gateway to view and analyze user request logs.
...
- [x] An exposed GET method in the Lambda function.
...
- [x] Create an event with Amazon CloudWatch Events that will monitor the S3 bucket and then insert the records into DynamoDB.
...
- [x] Save the picture's S3 key in an Amazon DynamoDB table. Use a function to generate a presigned URL every time an employee logs in. Return the URL to the browser.
...
- [x] Use Amazon SQS as an API message bus.
...
- [x] A Dockerfile in an application package.
...
- [x] Use AWS KMS to encrypt traffic between CloudFront and the web application.
...
- [x] Use the SyncCallback interface to receive notifications on the application.
...
- [x] Retrieve the messages from the queue using short polling every 10 seconds.
...
- [x] Modify the application to use Amazon S3 for serving shared images; cache data can then be written to local disks.
